...
public class SettlementService : ISettlementService
{
    ...
    public SettlementService(AppDbContext db, IAppNotificationService notificationService, ILogger<SettlementService> logger)
    {
        _db = db;
    ...
    public async Task<PaymentResponse> RecordPaymentAsync(Guid tripId, Guid requestingUserId, RecordPaymentRequest request)
    {
        ...
        // "X paid you back" — best-effort and deliberately swallowed: the payment
        // itself is already committed above and is the record that actually
        // matters, so a notification-write failure (e.g. a transient DB hiccup)
        // should never surface as a failed payment request. See
        // AppNotificationService.NotifyPaymentReceivedAsync for why this also
        // no-ops silently (not an error) when the recipient has no linked account.
        try
        {
        ...
        {
            _logger.LogWarning(ex, "Failed to create payment-received notification for payment {PaymentId}", payment.Id);
        }
