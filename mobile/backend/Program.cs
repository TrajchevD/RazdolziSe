...
builder.Services.AddScoped<ITripInviteService, TripInviteService>();
...
// ---- CORS so both the Angular web frontend and the Capacitor native shell can
// call this API. Split on comma into multiple origins (WithOrigins takes a
// params array) — a native app has no single "domain" to allow, so this always
// includes capacitor://localhost (iOS) and https://localhost (Android, per
// capacitor.config.ts's androidScheme) alongside whatever web URL(s) config adds.
// See appsettings.json's Cors:AllowedOrigins _comment for why those two stay
// even in production. ----
var allowedOrigins = (builder.Configuration["Cors:AllowedOrigins"] ?? "http://localhost:4200,capacitor://localhost,https://localhost")
    .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
builder.Services.AddCors(options =>
{
    options.AddPolicy("Frontend", policy =>
    {
        // AllowCredentials is required because identity.service.ts's bootstrap()/me()
        // calls (and the auth JWT flow) go out with withCredentials: true — the
        // anonymous-web-identity cookie (see identity-bootstrap.ts) only works if the
        // browser is allowed to send/receive it cross-origin. Note this is only valid
        // together with an explicit origin allowlist (WithOrigins above) — it's
        // incompatible with AllowAnyOrigin, which is why that's never used here.
